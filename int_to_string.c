...
/**
 * get_numbase_len - it gets length of buffer needed for an unsigned int
...
int get_numbase_len(unsigned int num, unsigned int base)
{
	int len = 1; /* all the numbers contain at least one digit/ value */
...
/**
 * fill_numbase_buff - it fills buffer with correct numbers up to base 36
 * @num: the number to convert to string given base
 * @base: the base of number used in conversion, only works up to base 36
 * @buff: the buffer to fill with result of the conversion
 * @buff_size: the size of buffer (bytes)
 *
 * Return: always void.
 */
void fill_numbase_buff(unsigned int num, unsigned int base,
			char *buff, int buff_size)
{
	...
	{
		rem = num % base;
		if (rem > 9) /* return to lowercase ascii val representation */
